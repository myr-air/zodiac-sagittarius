...
export const dayGroupRowClassName = "[&_td]:rounded-(--radius-sm)! [&_td]:border! [&_td]:border-(--color-route-border)! [&_td]:bg-(--color-route-soft)! [&_td]:px-3! [&_td]:py-2! [&_td]:text-xs [&_td]:font-black [&_td]:text-(--color-route) [&_td]:shadow-none! [&_strong]:float-right [&_strong]:tabular-nums";
export const ledgerSourceClassName = "text-[11px] font-bold text-(--color-text-muted)";
export const ledgerCalculationClassName = "block max-w-[360px] text-[11px] font-semibold leading-4 text-(--color-text-muted)";
