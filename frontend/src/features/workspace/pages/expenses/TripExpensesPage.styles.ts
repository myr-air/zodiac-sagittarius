...
export const ledgerDisclosureClassName = "max-w-[430px] rounded-(--radius-sm) border border-(--color-border) bg-(--color-surface-subtle) px-2 py-1.5 text-[11px] font-semibold leading-4 text-(--color-text-muted) [&_summary]:cursor-pointer [&_summary]:font-black [&_summary]:text-(--color-text)";
export const ledgerDetailsListClassName = "mt-2 grid gap-2 [&_dd]:m-0 [&_dd]:leading-4 [&_dt]:text-[10px] [&_dt]:font-black [&_dt]:uppercase [&_dt]:text-(--color-text-muted)";
export const ledgerMemberListClassName = "grid list-disc gap-1 pl-4";
export const ledgerSplitCellClassName = "grid max-w-[260px] gap-2";
...
export const actionCellClassName = "inline-flex min-w-[146px] flex-wrap items-center justify-end gap-1.5";
