...
export const memberStatGridClassName = "member-stat-grid grid grid-cols-5 gap-2 max-[1199px]:grid-cols-3 max-[767px]:grid-cols-2";
export const memberStatClassName = "member-stat grid min-h-14 grid-cols-[24px_minmax(0,1fr)] items-center gap-x-2 rounded-(--radius-md) border border-(--color-border) bg-(--color-surface) px-3 py-2 text-left shadow-none [&_.icon]:row-span-2 [&_.icon]:text-(--color-primary) [&>span]:text-[11px] [&>span]:font-bold [&>span]:leading-4 [&>span]:text-(--color-text-muted) [&>strong]:text-sm [&>strong]:font-black [&>strong]:leading-5 [&>strong]:tabular-nums [&>strong]:text-(--color-text)";
export const memberCommandBarClassName = `member-command-bar grid min-w-0 gap-2 rounded-(--radius-lg) border border-(--color-border) bg-(--color-surface) p-3 shadow-none max-[1199px]:grid-cols-1 ${workspaceResponsivePanelResetClassName}`;
