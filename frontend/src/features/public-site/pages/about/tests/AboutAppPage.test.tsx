...
describe("AboutAppPage", () => {
  it("shows web and API version details without exposing raw configuration", async () => {
    ...
    expect(screen.getByText("Joii v0.3.0")).toBeInTheDocument();
    expect(await screen.findByText("Joii v0.2.0")).toBeInTheDocument();
  ...
  it("keeps the page usable when API version details are unavailable", async () => {
    ...
    expect(await screen.findByText("Joii v0.3.0")).toBeInTheDocument();
    await waitFor(() => {
