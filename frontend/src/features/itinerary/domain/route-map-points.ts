...
export function hasCoordinates(
  coordinate: ItineraryItem["coordinates"],
): coordinate is NonNullable<ItineraryItem["coordinates"]> {
  return Boolean(
    coordinate
...
export function buildRoutePoints(items: ItineraryItem[]): RoutePoint[] {
  ...
  const coordinateItems = regionalItems.filter((item) =>
  ...
  return regionalItems.map((item, index) => {
    const point =
      item.coordinates && bounds
        ? projectCoordinate(item.coordinates, bounds)
        : fallbackPoint(item, regionalItems, index);
    return { item, ...point };
  });
}
...
function fallbackPoint(
  ...
  const dayIndex = uniqueStrings(items.map((candidate) => candidate.day)).indexOf(
    item.day,
  );
  const dayItems = items.filter((candidate) => candidate.day === item.day);
  const indexInDay = Math.max(
    0,
    dayItems.findIndex((candidate) => candidate.id === item.id),
  );
