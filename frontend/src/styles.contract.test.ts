...
describe("Calm Travel Ops CSS contract", () => {
  ...
  it("renders itinerary header controls as an animated overlay", () => {
    ...
    expect(smartTableSource).toContain("w-[min(640px,calc(100vw_-_32px))]");
    expect(smartTableSource).toContain("grid-cols-[minmax(220px,1fr)_minmax(150px,180px)]");
    expect(smartTableSource).toContain("min-h-10 w-full min-w-0 rounded-(--radius-sm)");
    expect(smartTableSource).toContain("itinerary-filter-panel grid min-w-0 grid-cols-[repeat(auto-fit,minmax(118px,1fr))]");
  ...
  it("uses an edge-to-edge three-line mobile itinerary row layout", () => {
    ...
    expect(smartTableSource).not.toContain("max-[360px]:grid-cols-1");
    expect(smartTableSource).not.toContain("max-[360px]:col-start-auto");
    expect(smartTableSource).toContain("max-[520px]:w-full max-[520px]:max-w-full");
    ...
    expect(css).toMatch(/\.smart-table,[\s\S]*?\.smart-table td\s*{[\s\S]*?display:\s*block;[\s\S]*?width:\s*100%;[\s\S]*?border-right:\s*0 !important;/);
    ...
    expect(css).toMatch(/\.smart-table \.item-placeholder-cell\s*{[\s\S]*?padding:\s*0 !important;[\s\S]*?width:\s*100%;/);
  });
