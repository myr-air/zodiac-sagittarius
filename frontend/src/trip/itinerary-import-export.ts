...
export { buildItineraryExport } from "./itinerary-export-builders";
...
export function parseItineraryImportDocument(source: string): ItineraryExportDocument {
  ...
  if (!looksLikeJsonImport(normalizedSource)) {
    return parseSpreadsheetItineraryImportDocument(normalizedSource);
  }
...
function parseSpreadsheetItineraryImportDocument(source: string): ItineraryExportDocument {
  ...
  return {
    ...
    items: normalizeImportedHierarchy(parsed.items),
