import type { ItineraryAdvisory, ItineraryCoordinates, ItineraryItem, Trip } from "./types";
...
export interface ItineraryExportDocument {
  ...
  source?: "json" | "ai";
  exportedAt: string;
  trip: Pick<Trip, "id" | "name" | "destinationLabel" | "startDate" | "endDate" | "activePlanVariantId">;
  items: ItineraryExportItem[];
}

export function buildItineraryExport({ exportedAt, items, trip }: { exportedAt: string; items: ItineraryItem[]; trip: Trip }): ItineraryExportDocument {
  return {
...
export function parseItineraryImport(source: string): ItineraryExportItem[] {
  ...
  if (!isRecord(parsed) || parsed.schema !== itineraryExportSchema || parsed.version !== itineraryExportVersion || !Array.isArray(parsed.items)) {
...
function parseExportItem(value: unknown): ItineraryExportItem {
  ...
  return {
    ...
    durationMinutes: item.durationMinutes === null ? null : readNumber(item, "durationMinutes"),
...
function readOptionalString(item: Record<string, unknown>, key: string): string | undefined {
  ...
  if (typeof value !== "string") throw new Error("Unsupported itinerary import file.");
  return value;
}

function readOptionalPathRole(item: Record<string, unknown>, key: string): ItineraryItem["pathRole"] | undefined {
  const value = item[key];
  if (value === undefined || value === null) return undefined;
...
function readNumber(item: Record<string, unknown>, key: string): number {
  const value = item[key];
  if (typeof value !== "number" || !Number.isFinite(value)) throw new Error("Unsupported itinerary import file.");
  return value;
}

function readActivityType(value: unknown): ItineraryItem["activityType"] {
  if (value === "travel" || value === "food" || value === "shopping" || value === "attraction" || value === "experience" || value === "stay") return value;
  throw new Error("Unsupported itinerary import file.");
}

function readCoordinates(value: unknown): ItineraryCoordinates | undefined {
  if (value === undefined) return undefined;
  if (!isRecord(value) || typeof value.lat !== "number" || typeof value.lng !== "number") throw new Error("Unsupported itinerary import file.");
  return { lat: value.lat, lng: value.lng };
}

function readAdvisories(value: unknown): ItineraryAdvisory[] | undefined {
  if (value === undefined) return undefined;
  if (!Array.isArray(value)) throw new Error("Unsupported itinerary import file.");
  return value.map((entry) => {
    if (!isRecord(entry) || typeof entry.code !== "string" || typeof entry.label !== "string") throw new Error("Unsupported itinerary import file.");
    if (entry.severity !== "info" && entry.severity !== "warning" && entry.severity !== "critical") throw new Error("Unsupported itinerary import file.");
