    ...
    <section className={panelClassName} aria-label={t.suggestions.queueLabel}>
      <div className={titleRowClassName}>
        <h3 className={titleClassName}>
          {t.suggestions.title({ count: openSuggestions.length })}
        </h3>
        <button className={titleButtonClassName} type="button">
          {t.suggestions.seeMore}
        </button>
      </div>
      <div className={listClassName}>
        {openSuggestions.map((suggestion) => {
          const proposer = members.find(
          ...
          return (
            ...
            >
              ...
              <div className={copyClassName}>
                <strong className={itemTitleClassName}>
                  {suggestion.proposedPatch.activity ?? t.suggestions.fallback}
                </strong>
                <span className={itemMetaClassName}>
                  {t.suggestions.suggestedUpdate({
