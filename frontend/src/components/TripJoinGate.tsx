...
const joinShellClassName = "join-shell grid w-[min(100%,860px)] gap-[18px] rounded-(--radius-lg) border border-(--color-border) bg-white/90 p-[22px] shadow-[var(--shadow-panel)] max-[767px]:p-4";
...
const joinAlertStackClassName =
  "join-alert-stack pointer-events-none fixed top-4 right-[clamp(18px,4vw,44px)] left-auto z-[60] grid w-[min(430px,calc(100vw-36px))] gap-2.5 max-[767px]:top-auto max-[767px]:right-0 max-[767px]:left-0 max-[767px]:bottom-5 max-[767px]:px-[18px] [&_.join-alert]:pointer-events-auto [&_.join-alert]:min-h-12 [&_.join-alert]:w-full [&_.join-alert]:justify-start [&_.join-alert]:rounded-(--radius-lg) [&_.join-alert]:shadow-[0_18px_44px_rgb(15_23_42_/_0.14)]";
...
const tripAccessSubmitClassName = "!min-h-[52px] !rounded-xl !mt-5 shadow-[0_16px_30px_rgb(15_118_110_/_0.18)]";
...
const tripAccessNotesClassName =
  "trip-access-notes relative z-[1] m-0 mt-[18px] grid list-none gap-2.5 p-0 max-[767px]:hidden [&_.icon]:size-[34px] [&_.icon]:rounded-[10px] [&_.icon]:bg-(--color-primary-soft) [&_.icon]:p-2 [&_.icon]:text-(--color-primary-strong) [&_li]:grid [&_li]:grid-cols-[34px_minmax(0,1fr)] [&_li]:items-center [&_li]:gap-2.5 [&_li]:rounded-[14px] [&_li]:border [&_li]:border-[rgb(226_232_240_/_0.82)] [&_li]:bg-[rgb(255_255_255_/_0.82)] [&_li]:p-3 [&_small]:block [&_small]:text-xs [&_small]:font-[650] [&_small]:leading-[17px] [&_small]:text-(--color-text-muted) [&_strong]:block [&_strong]:text-[13px] [&_strong]:leading-[18px] [&_strong]:text-(--color-text)";
