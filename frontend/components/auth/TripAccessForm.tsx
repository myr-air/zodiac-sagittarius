...
export function TripAccessForm() {
  ...
  return (
    <div className="w-full">
      <p className="m-0 mb-3 inline-flex items-center gap-2 rounded-full border border-(--color-primary-border) bg-(--color-primary-soft) px-3 py-1 text-xs font-bold text-(--color-primary-strong)">
        <span
      ...
      <h1 className="m-0 text-[clamp(1.618rem,3vw,2.125rem)] font-bold leading-tight text-(--color-text)">
        {copy.trip.heading}
      </h1>
          ...
          <div className="grid gap-2">
            ...
            >
              <input
                ...
                disabled={pending}
                className="min-h-14 w-full rounded-(--radius-sm) border border-(--color-border) bg-(--color-surface) px-3 font-mono tracking-[0.06em] text-(--color-text) outline-none transition-[border-color,box-shadow] duration-[180ms] focus:border-(--color-primary) focus:shadow-[0_0_0_3px_rgba(15,118,110,0.18)]"
              />
              <button
                type="submit"
                disabled={pending || code.trim().length === 0}
                className="min-h-14 w-full rounded-(--radius-sm) bg-(--color-primary) px-4 font-bold text-(--color-on-primary) transition-colors duration-[180ms] hover:bg-(--color-primary-strong) disabled:opacity-60"
              >
                {copy.trip.enter}
              </button>
            </div>
        ...
        <form className="mt-6 grid w-full gap-4" onSubmit={onSubmitClaim}>
          <div className="grid gap-2">
            ...
            <select
              ...
              disabled={pending}
              className="min-h-14 w-full rounded-(--radius-sm) border border-(--color-border) bg-(--color-surface) px-3 text-(--color-text) outline-none duration-[180ms] focus:border-(--color-primary) focus:shadow-[0_0_0_3px_rgba(15,118,110,0.18)]"
            >
      ...
      {error ? (
        <p
          role="alert"
          className="mt-4 rounded-(--radius-md) border border-(--color-danger-border) bg-(--color-danger-soft) px-3 py-2 text-sm text-(--color-danger)"
        >
          {error}
        </p>
      ) : null}
      ...
      {phase.kind === "code" ? (
        <p className="mt-5 mb-0 rounded-(--radius-md) border border-(--color-route-border) bg-(--color-route-soft) px-3 py-3 text-[0.8125rem] leading-[1.618] text-(--color-text-muted)">
          <strong className="font-bold text-(--color-route)">
            {copy.trip.tipLead}
          </strong>{" "}
          {copy.trip.tipBeforeLogin}{" "}
          <a
            href={shell.loginHref}
            className="inline font-bold text-(--color-route) underline-offset-2 hover:underline"
          >
            {copy.trip.logInLink}
          </a>{" "}
          {copy.trip.tipAfterLogin}
        </p>
      ) : null}
