...
db-init: db-create
	...
	@if ! $(PSQL) "$(DATABASE_URL)" -tAc "SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='expenses' AND column_name='spent_on'" | grep -q 1; then \
	  $(PSQL) -v ON_ERROR_STOP=1 "$(DATABASE_URL)" < backend/migrations/0033_expense_spent_on.sql; \
	fi
	@if ! $(PSQL) "$(DATABASE_URL)" -tAc "SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='expenses' AND column_name='stored_value_transaction_type'" | grep -q 1; then \
	  $(PSQL) -v ON_ERROR_STOP=1 "$(DATABASE_URL)" < backend/migrations/0034_expense_stored_value_cards.sql; \
	fi
...
db-init-test: db-create-test
	...
	@if ! $(PSQL) "$(TEST_DATABASE_URL)" -tAc "SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='expenses' AND column_name='spent_on'" | grep -q 1; then \
	  $(PSQL) -v ON_ERROR_STOP=1 "$(TEST_DATABASE_URL)" < backend/migrations/0033_expense_spent_on.sql; \
	fi
	@if ! $(PSQL) "$(TEST_DATABASE_URL)" -tAc "SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='expenses' AND column_name='stored_value_transaction_type'" | grep -q 1; then \
	  $(PSQL) -v ON_ERROR_STOP=1 "$(TEST_DATABASE_URL)" < backend/migrations/0034_expense_stored_value_cards.sql; \
	fi
