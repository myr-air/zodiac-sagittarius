...
pub async fn join_trip(
    ...
    enforce_auth_attempt_not_locked(&mut tx, TRIP_JOIN_ATTEMPT_SCOPE, &normalized_join_id, now).await?;
...
pub async fn verify_join_session(
    ...
    db::queries::find_active_trip_join_session(pool, trip_id, &token_hash).await?
        .ok_or(ServiceError::Unauthenticated)?;
...
pub async fn claim_member(
    ...
    let member = db::queries::lock_member(&mut tx, trip_id, member_id).await?
        .ok_or(ServiceError::NotFound)?;
    ...
    match member.claim_password_hash.as_deref() {
        ...
        None => {
            ...
            db::queries::set_member_claim_password(&mut tx, trip_id, member_id, &password_hash).await?;
        }
    }
...
pub fn hash_session_token(session_token: &str) -> Result<String, ServiceError> {
...
pub(crate) async fn create_session(
    ...
    let policy = db::queries::find_member_session_policy(tx, trip_id, member_id).await?
        .ok_or(ServiceError::Unauthenticated)?;
