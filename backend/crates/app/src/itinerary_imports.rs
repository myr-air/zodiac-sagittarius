...
use sagittarius_domain::patches::ImportItineraryRequest;
...
pub async fn import_itinerary(
    ...
    let trip_plans: Vec<_> = plan_variants
        .into_iter()
        .map(|record| {
            sagittarius_db::models::plan_variant_summary_for_main_pointer(record, Some(active_plan_variant_id))
        })
        .collect();
...
fn itinerary_json_schema() -> Value {
    ...
    let item_schema = json!({
        "type": "object",
        "additionalProperties": false,
        "required": ["id", "day", "sortOrder", "startTime", "activity", "activityType", "activitySubtype", "place", "mapLink", "durationMinutes", "transportation", "details", "note"],
        "properties": {
            "id": { "type": "string" },
