...
use sagittarius_db::models::{NewSuggestion, SuggestionRecord};
use sagittarius_domain::capabilities::can;
use sagittarius_domain::errors::ServiceError;
use sagittarius_domain::patches::{CreateSuggestionRequest, ItineraryItemPatch};
use sagittarius_domain::types::{Capability, SuggestionSummary};
...
pub async fn create_suggestion(
    ...
    let suggestion = db::queries::insert_suggestion(
        ...
        NewSuggestion {
            ...
            trip_plan_id: request.plan_variant_id,
...
pub async fn approve_suggestion(
    ...
    if item.trip_plan_id != suggestion.trip_plan_id {
        return Err(ServiceError::InvalidRequest(
...
async fn validate_suggestion_references(
    ...
    if target.trip_plan_id != request.plan_variant_id {
        return Err(ServiceError::InvalidRequest(
...
async fn update_status_and_insert_event(
    ...
    let updated = db::queries::update_suggestion_status(tx, suggestion.id, status, resolved_by).await?
        .ok_or(ServiceError::NotFound)?;
