...
use sagittarius_db::models::NewTripTask;
use sagittarius_domain::capabilities::can;
use sagittarius_domain::errors::ServiceError;
use sagittarius_domain::patches::{CreateTaskRequest, PatchTaskRequest, TaskPatch};
use sagittarius_domain::types::{Capability, TripRole, TripTaskSummary};
...
pub async fn create_task(
    ...
    let mut mutation = kernel::MemberMutation::begin_with(
        pool,
        trip_id,
        session_token,
        required_capability,
    ...
    let task = db::queries::insert_task(
        ...
        NewTripTask {
            id: Uuid::now_v7(),
            trip_id: mutation.session.trip_id,
            trip_plan_id,
            title: request.title.trim(),
            visibility: request.visibility.as_str(),
            kind: request.kind.as_deref(),
            created_by: mutation.session.member_id,
            assignee_id,
...
fn can_patch_task(
    role: TripRole,
    member_id: Uuid,
    task: &sagittarius_db::models::TripTaskRecord,
) -> bool {
    match role {
...
async fn resolve_task_patch_trip_plan_id(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    trip_id: Uuid,
    existing: &sagittarius_db::models::TripTaskRecord,
    patch: &TaskPatch,
) -> Result<Option<Uuid>, ServiceError> {
...
mod tests {
    use super::*;
    use sagittarius_db::models::TripTaskRecord;
