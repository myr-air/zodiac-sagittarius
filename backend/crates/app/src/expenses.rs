...
use crate::{auth, events, kernel, mutation_guard, trips};
use sagittarius_db as db;
use sagittarius_db::PgPool;
use sagittarius_db::models::{ExpenseReminderRecord, NewExpense, NewExpenseReminder};
use sagittarius_domain::capabilities::can;
use sagittarius_domain::errors::ServiceError;
use sagittarius_domain::expense_patch_rules::validate_stored_value_fields;
...
use sagittarius_domain::types::{Capability, ExpenseItemSummary, ExpenseSummary};
...
pub async fn get_expense_summary(
    ...
    let session =
        kernel::require_member_session(pool, trip_id, session_token, Capability::ViewExpenses)
            .await?;
...
pub async fn create_expense(
    ...
    let mut mutation = kernel::MemberMutation::begin(
        ...
        trip_id,
        session_token,
        &[Capability::CreateExpense, Capability::EditExpenses],
