...
use crate::{auth, bookings, events, mutation_guard, photo_albums, plan_checks};
use sagittarius_db as db;
use sagittarius_db::PgPool;
use sagittarius_db::models::{ExpenseReminderRecord, ExpenseSplitRecord};
use sagittarius_domain::capabilities::can;
use sagittarius_domain::errors::ServiceError;
use sagittarius_domain::patches::PatchTripRequest;
...
pub async fn load_cockpit(
    ...
    if matches!(
        session.role,
        sagittarius_domain::types::TripRole::Organizer | sagittarius_domain::types::TripRole::Traveler
    ) {
    ...
    let plan_summaries: Vec<_> = plan_variants
        .into_iter()
        .map(|record| {
            sagittarius_db::models::plan_variant_summary_for_main_pointer(
                record,
