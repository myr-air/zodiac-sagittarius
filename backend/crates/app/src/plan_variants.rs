...
use sagittarius_domain::types::{Capability, PlanVariantSummary, TripSummary};
...
pub async fn patch_plan_variant(
    ...
    if existing.version != request.expected_version {
        return Err(mutation_guard::version_conflict_with_latest(
            sagittarius_db::models::plan_variant_summary_for_main_pointer(existing, main_trip_plan_id),
    ...
    .ok_or(ServiceError::NotFound)?;
    let mut updated =
        sagittarius_db::models::plan_variant_summary_for_main_pointer(updated_record.clone(), main_trip_plan_id);
    if updated.status != updated_record.status {
        ...
        .ok_or(ServiceError::NotFound)?;
        updated =
            sagittarius_db::models::plan_variant_summary_for_main_pointer(updated_record, main_trip_plan_id);
    }
    let event = insert_variant_event(
        &mut tx,
        &updated,
...
pub async fn publish_plan_variant(
    ...
    if let Some(previous_main_id) = trip.main_trip_plan_id {
