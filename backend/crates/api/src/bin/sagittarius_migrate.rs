...
mod tests {
    ...
    #[test]
    fn pending_migrations_reject_unknown_applied_version() {
        let migrations = vec![Migration {
            version: "0001_baseline.sql",
            sql: "select 1;",
        }];
        let applied = BTreeMap::from([(
            "0035_expense_settlement_allocations.sql".to_string(),
            checksum_hex("select 1;".as_bytes()),
        )]);

        let error = pending_migrations(&migrations, &applied).expect_err("unknown applied");

        assert!(
            error
                .to_string()
                .contains("0035_expense_settlement_allocations.sql")
        );
    }

    #[test]
    fn embedded_migrations_include_baseline() {
        ...
        assert!(versions.contains("0001_baseline.sql"));
    }
