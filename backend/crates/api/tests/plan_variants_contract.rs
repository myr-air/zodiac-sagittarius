...
async fn trip_plan_contract_accepts_canonical_routes_and_status(pool: sqlx::PgPool) {
    ...
    let previous_status: String =
        sqlx::query_scalar("select status from trip_plans where id = $1")
...
async fn trip_plan_contract_rejects_main_status_and_empty_patch(pool: sqlx::PgPool) {
    ...
    let plan_version: i64 = sqlx::query_scalar("select version from trip_plans where id = $1")
        .bind(Uuid::parse_str(support::PLAN_ID).unwrap())
        .fetch_one(&pool)
        .await
        .unwrap();
...
async fn trip_plan_contract_rejects_invalid_set_main_status_without_writes(pool: sqlx::PgPool) {
    ...
    let active_plan_variant_id: Uuid =
        sqlx::query_scalar("select main_trip_plan_id from trips where id = $1")
            .bind(Uuid::parse_str(support::TRIP_ID).unwrap())
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn trip_plan_contract_rejects_missing_and_cross_trip_targets_without_writes(
    ...
    let initial_main: Uuid =
        sqlx::query_scalar("select main_trip_plan_id from trips where id = $1")
            ...
            .await
            .unwrap();
    let initial_main_status: (String, i64) =
        sqlx::query_as("select status, version from trip_plans where id = $1")
            .bind(Uuid::parse_str(support::PLAN_ID).unwrap())
            .fetch_one(&pool)
            .await
            .unwrap();
    ...
    let current_main: Uuid =
        sqlx::query_scalar("select main_trip_plan_id from trips where id = $1")
            ...
            .await
            .unwrap();
    let current_main_status: (String, i64) =
        sqlx::query_as("select status, version from trip_plans where id = $1")
            .bind(Uuid::parse_str(support::PLAN_ID).unwrap())
            .fetch_one(&pool)
            .await
...
async fn trip_plan_contract_set_main_honors_previous_statuses_and_duplicate_guard(
    ...
    let previous_main_status: String =
        sqlx::query_scalar("select status from trip_plans where id = $1")
            .bind(Uuid::parse_str(support::PLAN_ID).unwrap())
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(previous_main_status, "proposal");
    ...
    let proposal_target_status: String =
        sqlx::query_scalar("select status from trip_plans where id = $1")
            .bind(proposal_target_id)
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(proposal_target_status, "draft");
}
...
async fn plan_variant_contract_traveler_cannot_manage_trip_plans(pool: sqlx::PgPool) {
    ...
    let initial_target: (String, Option<String>, i64) =
        sqlx::query_as("select name, status, version from trip_plans where id = $1")
            .bind(alt_plan_id)
            .fetch_one(&pool)
            .await
            .unwrap();
