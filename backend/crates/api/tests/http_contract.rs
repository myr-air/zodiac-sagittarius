...
async fn database_errors_do_not_leak_details() {
    let response = sagittarius_api::api::error::ApiError(ServiceError::Database(
...
async fn account_conflict_errors_return_stable_codes() {
    let identity_response =
        sagittarius_api::api::error::ApiError(ServiceError::IdentityAlreadyLinked).into_response();
    ...
    let owner_response =
        sagittarius_api::api::error::ApiError(ServiceError::OwnerTransferInvalid).into_response();
