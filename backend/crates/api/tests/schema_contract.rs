...
async fn itinerary_end_offset_requires_end_time(pool: sqlx::PgPool) {
    ...
    sqlx::query(
        "insert into itinerary_items (
           id, trip_id, trip_plan_id, day, sort_order, start_time, end_time,
...
async fn itinerary_schema_rejects_parent_outside_child_day_or_plan(pool: sqlx::PgPool) {
    ...
    sqlx::query(
        "insert into trip_plans (id, trip_id, name, status)
         values ($1, $2, 'Backup', 'backup')",
    )
    .bind(other_plan_id)
    ...
    sqlx::query(
        "insert into itinerary_items (
           id, trip_id, trip_plan_id, day, sort_order, start_time,
    ...
    let child_result = sqlx::query(
        "insert into itinerary_items (
           id, trip_id, trip_plan_id, parent_item_id, day, sort_order,
...
#[sqlx::test(migrations = "../../migrations")]
async fn baseline_schema_uses_trip_plans_without_legacy_columns(pool: sqlx::PgPool) {
    let trip_plan_columns: Vec<String> = sqlx::query_scalar(
    ...
    .await
    .unwrap();
    assert!(trip_plan_columns.contains(&"status".to_string()));
    assert!(!trip_plan_columns.contains(&"kind".to_string()));

    let trip_columns: Vec<String> = sqlx::query_scalar(
        "select column_name::text
         from information_schema.columns
         where table_schema = 'public'
           and table_name = 'trips'
           and column_name in ('main_trip_plan_id', 'active_plan_variant_id')
         order by column_name",
    )
    .fetch_all(&pool)
    .await
    .unwrap();
    assert_eq!(trip_columns, vec!["main_trip_plan_id".to_string()]);

    let itinerary_columns: Vec<String> = sqlx::query_scalar(
        "select column_name::text
         from information_schema.columns
         where table_schema = 'public'
           and table_name = 'itinerary_items'
           and column_name in ('trip_plan_id', 'plan_variant_id', 'link_label', 'advisories')
         order by column_name",
    )
    .fetch_all(&pool)
    .await
    .unwrap();
    assert_eq!(itinerary_columns, vec!["trip_plan_id".to_string()]);

    let parent_fks: Vec<String> = sqlx::query_scalar(
        "select conname::text
         from pg_constraint
    ...
    assert_eq!(
        parent_fks,
        vec!["itinerary_items_parent_scope_fkey".to_string()]
    );
}
...
async fn trip_plan_compatibility_schema_adds_status_and_keeps_composite_identity(
    ...
    assert_eq!(
        columns,
        vec![("status".to_string(), "text".to_string(), "NO".to_string(),)],
    );
    ...
    .await
    .unwrap();
    let status_check = status_check.expect("missing trip_plans_status_check");
    ...
    let composite_unique_count: i64 = sqlx::query_scalar(
        "select count(*)
         from pg_constraint
         where conrelid = 'trip_plans'::regclass
           and contype in ('p', 'u')
           and conkey = ARRAY[
             (select attnum from pg_attribute where attrelid = 'trip_plans'::regclass and attname = 'id'),
             (select attnum from pg_attribute where attrelid = 'trip_plans'::regclass and attname = 'trip_id')
           ]::smallint[]",
    )
    .fetch_one(&pool)
    .await
    ...
    assert!(
        ...
        "trip_plans(id, trip_id) must stay unique for composite FKs",
    );
}
...
async fn trips_cycle_foreign_keys_are_deferred(pool: sqlx::PgPool) {
    let constraints: Vec<(String, bool, bool)> = sqlx::query_as(
        "select conname::text, condeferrable, condeferred
         from pg_constraint
         where conname in ('trips_owner_member_id_fkey', 'trips_main_trip_plan_id_fkey')
         order by conname",
    )
    .fetch_all(&pool)
    .await
    ...
    assert_eq!(
        constraints,
        vec![
            ("trips_main_trip_plan_id_fkey".to_string(), true, true),
...
async fn same_trip_composite_keys_reject_cross_trip_plan_reference(pool: sqlx::PgPool) {
    ...
    let result = sqlx::query(
        "insert into itinerary_items (
           id, trip_id, trip_plan_id, day, sort_order, activity, activity_type, place, created_by
         )
...
async fn insert_trip(
    ...
    sqlx::query(
        "insert into trips (
           ...
           main_trip_plan_id, owner_member_id
         )
