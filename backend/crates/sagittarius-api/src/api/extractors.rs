...
use crate::{api::CorsOriginPolicy, app::AppState, domain::errors::ServiceError};
...
fn bearer_token_from_parts(parts: &Parts) -> Result<BearerToken, ServiceError> {
    let header = parts
        .headers
...
fn validate_cookie_account_request(parts: &Parts) -> Result<(), ServiceError> {
    if matches!(
        &parts.method,
...
pub(crate) fn validate_account_cookie_origin(parts: &Parts) -> Result<(), ServiceError> {
    let origin = parts
        .headers
        .get(header::ORIGIN)
        .and_then(|value| value.to_str().ok())
...
mod tests {
    ...
    async fn account_session_token_requires_origin_for_unsafe_cookie_requests() {
        ...
        assert!(matches!(missing_origin, Err(ServiceError::Forbidden)));
