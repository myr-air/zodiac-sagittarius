...
#[sqlx::test(migrations = "../../migrations")]
async fn booking_create_unlinked_uses_requested_trip_plan(pool: sqlx::PgPool) {
    support::seed_trip(&pool).await;
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(
                    json!({
                        "clientMutationId": "booking-alt-requested-plan",
                        "tripPlanId": support::ALT_PLAN_ID,
                        "type": "flight",
                        "title": "Draft rain route flight hold",
                        "status": "draft",
                        "visibility": "shared",
                        "ownerMemberId": support::ORGANIZER_ID,
                        "providerName": "HK Express",
    ...
    assert_eq!(body["relatedItineraryItemIds"], json!([]));
...
#[sqlx::test(migrations = "../../migrations")]
async fn booking_create_rejects_itinerary_items_from_multiple_trip_plans(pool: sqlx::PgPool) {
    support::seed_trip(&pool).await;
    let alt_item_id = support::seed_alt_plan_item(&pool).await;
    let organizer_token = support::create_session(&pool, support::ORGANIZER_ID).await;
    let app = support::app(pool.clone());
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(
                    json!({
                        "clientMutationId": "booking-create-cross-plan-items",
                        "type": "hotel",
                        "title": "Mixed plan hotel",
                        "status": "booked",
                        "visibility": "shared",
                        "ownerMemberId": support::ORGANIZER_ID,
                        "providerName": "Hotel",
                        "confirmationCode": "MIXED-HOTEL",
                        "startsAt": null,
                        "endsAt": null,
                        "timezone": "Asia/Hong_Kong",
                        "priceAmount": 500,
                        ...
                        "travelerIds": [support::TRAVELER_ID],
                        "externalLinks": [],
                        "relatedItineraryItemIds": [support::ITEM_ID, alt_item_id],
                        "relatedTaskIds": [],
                        "relatedExpenseIds": [],
                        "noteIds": [],
                        "notes": null
...
#[sqlx::test(migrations = "../../migrations")]
async fn booking_patch_rejects_itinerary_items_from_multiple_trip_plans(pool: sqlx::PgPool) {
    support::seed_trip(&pool).await;
    support::seed_booking_doc(&pool).await;
    let alt_item_id = support::seed_alt_plan_item(&pool).await;
    let organizer_token = support::create_session(&pool, support::ORGANIZER_ID).await;
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(
                    json!({
                        "clientMutationId": "booking-patch-cross-plan-items",
                        "expectedVersion": 1,
                        "patch": {
                            "relatedItineraryItemIds": [support::ITEM_ID, alt_item_id]
                        }
