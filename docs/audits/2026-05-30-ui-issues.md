...
> Archived UI fix report. This file is historical evidence, not the active issue
...
* **Root Cause**: ตัวแปร `accountHeroClassName` ใน [AccountAccessPanel.tsx](../../frontend/src/components/AccountAccessPanel.tsx) มีคลาส `[.account-shell--entry_&>*]:relative` ซึ่งไป override `absolute` ของ `AuthHighlights` ทำให้ตำแหน่งเสียและซ้อนทับกัน
...
* **Fix**: แก้ไขคลาสของ container ในไฟล์ต่อไปนี้ให้เป็น `grid-rows-[auto_minmax(0,1fr)]` เพื่อล็อกให้ track ต่างๆ ไม่ขยายยืดเกิน content จริง:
  - [SmartItineraryTable.tsx](../../frontend/src/components/SmartItineraryTable.tsx)
  - [TimelineView.tsx](../../frontend/src/components/TimelineView.tsx)
  - [TripMembersPage.tsx](../../frontend/src/components/TripMembersPage.tsx)
...
* **Symptom**: หน้า Landing Page บนมือถือ ยังแสดงปุ่ม "Start planning" / "Join trip" และแสดงกล่อง "Product Preview" (Tokyo-Kamakura card) ซึ่งยาวเกินไปสำหรับมือถือ ขัดแย้งกับหลักการใน [DESIGN.md](../../DESIGN.md)
* **Root Cause**: ขาด class คุม responsive บนหน้าจอขนาดเล็กสำหรับปุ่ม Hero และ Product Preview
* **Fix**: เพิ่ม class `max-[760px]:hidden` คุมส่วนของปุ่ม Hero และ Product Preview ในหน้า [HomeLanding.tsx](../../frontend/src/components/HomeLanding.tsx) เมื่อเปิดด้วย viewport มือถือจะถูกซ่อนตัวอย่างถูกต้องตามดีไซน์
...
* **Fix**: ครอบช่องกรอกรหัสผ่านและปุ่มรูปตาด้วย `<span className={passwordInputRowClassName}>` ในหน้า [TripJoinGate.tsx](../../frontend/src/components/TripJoinGate.tsx) ทำให้ปุ่มตาจัดอยู่ด้านขวามือในบรรทัดเดียวกันอย่างสวยงาม
...
* **Fix**: เพิ่ม class `max-[767px]:flex-nowrap` และ fade mask image `max-[767px]:[mask-image:linear-gradient(to_right,#000_82%,transparent)]` ในไฟล์ [AccountAccessPanel.tsx](../../frontend/src/components/AccountAccessPanel.tsx) ทำให้เลื่อนสไลด์แนวนอนได้คล่องตัวและจางหายที่ขอบขวาอย่างพรีเมียม
...
* **Fix**: ปรับปรุง [AccountAccessPanel.tsx](../../frontend/src/components/AccountAccessPanel.tsx) โดยใส่คลาส `border-solid` กำหนดรูปแบบเส้นขอบ และใช้เงื่อนไขแบบ Mutually Exclusive แยกแยะระหว่าง `border-[var(--color-primary)]` กับ `border-transparent` รวมถึงแยกสีข้อความชัดเจน
