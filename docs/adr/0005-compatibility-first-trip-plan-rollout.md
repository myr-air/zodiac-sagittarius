...
**Phase 0/1 freeze**: Phase 1 acceptance is blocked until [the implementation spec](../itinerary-trip-plan-phase-0-1-implementation-spec.md) lists the additive API request/response diffs for cockpit, create, patch, set-main, realtime, and import/export; the `plan_variants.status` migration DDL draft; and the exact tests that prove canonical-only, legacy-only, and mixed payloads all normalize to the same Trip Plan state.
