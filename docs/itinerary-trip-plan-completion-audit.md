...
| `REQ-EXP-01` | Actual Expenses are real money and must not automatically move to a new plan. | Proved for scope, summary, reminders, explicit API move of unlinked expenses, organizer-selected UI move of unlinked expenses, duplicate-as-estimate UI flow, and no automatic Main Plan moves; incomplete for cancel/refund workflows | `CONTEXT.md`; ADR `0003`; `expenses_contract.rs`; `plan_variants_contract.rs` set-main record-stability assertions; frontend API/app tests; expense copy clarified in UI/docs. | User-facing cancel/refund workflows are not yet fully implemented as a product workflow. |
...
cancel/refund workflows and full Phase 3 hierarchy/time browser QA remain later
or partially implemented slices.
