...
| `REQ-HIER-01` | Itinerary shape is Plan Day -> Activity -> Sub-activity, with one sub-activity level and Activity Blocks for grouped journeys. | Proved for import/browser display path; partly proved for full mutation policy | ADR `0004`; DDL `0027`; backend create/patch hierarchy contracts; frontend itinerary ordering and hierarchy tests; Storybook `HierarchyBlocks`; itinerary import browser QA on desktop/mobile. | Full Phase 3 service/UI sign-off still needs policy coverage for all move/reparent sequences. |
...
state is not yet fully proved because complete Phase 3 move/reparent policy
coverage and deeper paid/committed booking lifecycle workflows remain later
slices.
