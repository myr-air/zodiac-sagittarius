...
| `REQ-INLINE-01` | Itinerary page is the primary planning surface with inline edits and optional details hidden behind quick controls. | Partly proved; itinerary-first booking template quick-create and import browser surface are covered | `SmartItineraryTable.tsx` inline fields; `StopDialog.tsx`; itinerary booking template menu; current component tests; itinerary import browser QA. | Full browser QA for every quick-edit/control path and any deeper paid/committed booking lifecycle workflows remain separate Phase 3 slices. |
...
cross-day time, and imported record scoping. The full requested product end
state is not yet fully proved because full browser QA for every itinerary
quick-edit/control path and deeper paid/committed booking lifecycle workflows
remain later slices.
